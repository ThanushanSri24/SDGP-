...
import { Search, ChevronLeft, User, CheckCircle2, AlertTriangle, Map, Users, Bell, UserCircle, Plus } from 'lucide-react-native';
...
const StudentCard = ({ item }: any) => (
  <View style={styles.card}>
    ...
    <View style={styles.statusSection}>
      ...
      {item.status === 'Absent' ? <AlertTriangle color="#FACC15" size={24} /> : <CheckCircle2 color="#22C55E" size={24} />}
    </View>
  </View>
);

const NavButton = ({ icon, label, color, active }: any) => (
  <TouchableOpacity style={[styles.navItem, active && styles.activeNavItem]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.backButton}><ChevronLeft color="black" size={28} /></TouchableOpacity>
        <Text style={styles.headerTitle}>Students</Text>
        <View style={{ width: 28 }} />
      </View>
      <View style={styles.blueContent}>
        <View style={styles.searchSection}>
          <Search color="#94A3B8" size={20} /><TextInput style={styles.searchInput} placeholder="Search by student name..." />
        </View>
